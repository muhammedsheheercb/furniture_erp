...
const UNITS: { value: UnitType; label: string; hint: string }[] = [
  {
    value: "pcs",
    label: "Pieces (pcs)",
    hint: "Individual units — chairs, sofas, tables",
  },
  {
    value: "set",
    label: "Set",
    hint: "Grouped sets — dining sets, bedroom sets",
  },
  {
    value: "meters",
    label: "Meters (m)",
    hint: "Length — wood planks, curtain rods, pipes",
  },
  {
    value: "sq.meters",
    label: "Sq. Meters (m²)",
    hint: "Area — fabric, glass, board, veneer",
  },
  {
    value: "kg",
    label: "Kilograms (kg)",
    hint: "Weight — metal fittings, foam, hardware",
  },
  { value: "box", label: "Box", hint: "Boxed goods — screws, hinges, handles" },
  {
    value: "roll",
    label: "Roll",
    hint: "Rolled material — fabric rolls, edge banding",
  },
  {
    value: "liters",
    label: "Liters (L)",
    hint: "Liquids — paint, varnish, adhesive, polish",
  },
];
...
export default function ItemModal({
  ...
  useEffect(() => {
    if (mode === "opening_stock" && open) {
      fetch("/api/items")
        .then((r) => r.json())
        .then((d) => setExistingItems(d.data || d));
    }
  }, [mode, open]);
    ...
    >
      ...
      >
        ...
        {mode === "opening_stock" && !isEdit ? (
          <div>
            <label style={labelStyle}>{t("selectExistingItem")}</label>
            <SearchSelect
          ...
          >
            <div>
              <label style={labelStyle}>{t("itemNumber")}</label>
              <input
                readOnly
                disabled
                style={{
            ...
            <div>
              <label style={labelStyle}>{t("itemProductName")}</label>
              <input
                placeholder={t("eg3seaterFabricSofa")}
                style={inputStyle}
                {...register("name")}
              />
              {errors.name && (
        ...
        >
          ...
          <div>
            <label style={labelStyle}>{t("unitOfMeasure")}</label>
            <select
              style={{ ...inputStyle, cursor: "pointer" }}
              {...register("unit")}
              onChange={(e) => {
                setValue("unit", e.target.value);
                setSelectedUnit(e.target.value as UnitType);
              }}
            >
              {UNITS.map((u) => (
          ...
          >
            ...
            <div>
              <label style={labelStyle}>
                {mode === "opening_stock" ? "OPENING QTY" : "QUANTITY"} (
                {unitInfo?.value || "pcs"})
              </label>
              <input
                type="number"
        ...
        {mode === "opening_stock" && (
          <div>
            <label style={labelStyle}>{t("stockEntryReference")}</label>
            <input
              readOnly
              disabled
            ...
            />
            <p style={{ fontSize: 11, color: "#A89080", marginTop: 4 }}>
              {t("autogeneratedOpeningStockReference")}
            </p>
          </div>
        )}
      </form>
