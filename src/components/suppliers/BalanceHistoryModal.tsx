...
export default function BalanceHistoryModal({
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={`Balance History: ${entityName}`}
      size="lg"
    >
              ...
              history
                ...
                .map((item, idx) => (
                  <tr key={idx} className="hover:bg-gray-50">
                    ...
                    <td
                      className={`px-4 py-3 text-sm font-bold text-end whitespace-nowrap ${item.amount >= 0 ? "text-green-600" : "text-red-600"}`}
                    >
                      {item.amount >= 0 ? "+" : ""}
                    ...
                    <td className="px-4 py-3 text-sm">
                      <span
                        className={`px-2 py-1 rounded text-xs font-medium ${item.type === "payment" ? "bg-blue-100 text-blue-800" : "bg-gray-100 text-gray-800"}`}
                      >
