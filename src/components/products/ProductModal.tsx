...
function makeEmpty(mode: Mode): FormState {
  return {
    ...
    color: "",
    status: "active",
    isManufactured: mode === "manufacture",
    description: "",
    currentStock: 0,
...
function ChipInput({
  ...
  return (
    <div>
      <label className="block text-xs font-semibold text-[#7A6055] mb-1.5">
        {label}
      </label>
      <div className="flex flex-wrap gap-1.5 mb-2 min-h-8">
        {values.map((v) => (
          <span
            key={v}
            className="inline-flex items-center gap-1 bg-[#1B3A2D] text-white text-xs font-medium px-2.5 py-1 rounded-full"
          >
            {v}
            <button
              type="button"
        ...
        {values.length === 0 && (
          <span className="text-xs text-[#C5B8B0] italic self-center">
            {t("noneAddedYet")}
          </span>
        )}
      </div>
      <div className="flex gap-2">
        ...
        <button
          type="button"
          onClick={add}
          className="px-3 py-1.5 rounded-lg bg-[#E8F0EC] text-[#1B3A2D] text-sm font-semibold hover:bg-[#D0E4D8]"
        >
          {t("add")}
        </button>
      </div>
...
export default function ProductModal({
  ...
  useEffect(() => {
    ...
    if (product) {
      ...
      setForm({
        ...
        pricing: {
          materialCost:
            product.pricing?.materialCost ?? product.purchaseAmount ?? 0,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (mode === "direct") {
      ...
      if (form.pricing.laborCost < 0) {
        errs.laborCost = "Labour cost cannot be negative";
        errorTab = "pricing";
      }
      if (form.pricing.extraCost < 0) {
        errs.extraCost = "Overhead cost cannot be negative";
        errorTab = "pricing";
      }
    }
    ...
    const payload = {
      ...
      purchaseAmount: isDirect
        ? form.pricing.purchasePrice
        : form.pricing.totalCost,
      salesAmount: isDirect
        ? form.pricing.salesPrice
        : form.pricing.sellingPrice,
    ...
    >
      ...
      <form id="product-form" onSubmit={handleSubmit} className="min-h-96">
        ...
        {tab === "basic" && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className={lbl}>{t("currentStock")}</label>
                <input
                  type="number"
                  min={0}
                ...
                {(["width", "height", "depth"] as const).map((f) => (
                  <div key={f}>
                    ...
                    <input
                      type="number"
                      min={0}
                      value={(form.dimensions as any)[f]}
                      onChange={(e) =>
                        setForm((p) => ({
                          ...p,
        ...
        {tab === "pricing" && mode === "direct" && (
          <div className="space-y-5">
            <div className="p-4 rounded-xl bg-amber-50 border border-amber-200 flex gap-3 text-sm text-amber-800">
              ...
              />
              <span>{t("directBuyModeEnterThe")}</span>
            </div>
            <div className="grid grid-cols-2 gap-6">
        ...
        {tab === "variants" && (
          <div className="space-y-6">
            <p className="text-sm text-[#7A6055]">
              {t("addAvailableColorsSizesAnd")}
              <kbd className="px-1.5 py-0.5 bg-[#F5F2EA] rounded text-xs border border-[#E5DDD5] font-mono">
                {t("enter")}
              </kbd>{" "}
              {t("toAdd")}
            </p>
            <ChipInput
              label={t("colors")}
              values={form.variants.colors}
              onChange={(v) =>
                setForm((p) => ({
                  ...p,
                  variants: { ...p.variants, colors: v },
                }))
              }
            />
            <ChipInput
              label={t("sizes")}
              values={form.variants.sizes}
              onChange={(v) =>
                setForm((p) => ({
                  ...p,
                  variants: { ...p.variants, sizes: v },
                }))
              }
            />
            <ChipInput
              label={t("finishes")}
              values={form.variants.finishes}
              onChange={(v) =>
                setForm((p) => ({
                  ...p,
                  variants: { ...p.variants, finishes: v },
                }))
              }
            />
            ...
            {(form.variants.colors.length > 0 ||
              ...
              form.variants.finishes.length > 0) && (
              <div className="rounded-xl bg-[#FAF8F6] border border-[#E5DDD5] p-4 space-y-2">
                ...
                {["colors", "sizes", "finishes"].map((key) => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-[#A89080] w-16 text-xs capitalize">
                        {key}:
                      </span>
                      {vals.map((v) => (
                        <span
                          key={v}
                          className="px-2 py-0.5 rounded-full bg-white border border-[#E5DDD5] text-xs text-[#1A1210]"
                        >
                          {v}
                        </span>
                      ))}
