...
export default function ItemsPage() {
  ...
  const handleSubmit = async (data: any) => {
    ...
    if (editItem) {
      ...
      ok = await updateItem(data.itemId, {
        ...data,
    ...
    <div className="page-container">
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 13, color: "#7A6055", margin: "4px 0 0" }}>
            {total} {t("furnitureItemsMaterials")}
          </p>
        </div>
        {canCreate && (
          <div style={{ display: "flex", gap: 10 }}>
            ...
            >
              <Package size={15} /> {t("openingStock")}
            </motion.button>
            <motion.button
              whileHover={{ scale: 1.02, y: -1 }}
              whileTap={{ scale: 0.98 }}
      ...
      >
        ...
        >
          ...
          <input
            placeholder={t("searchByNameOrItem")}
            value={search}
        ...
        <table className="w-full">
          <thead>
            <tr>
              ...
              <th className="th text-center">{t("unit")}</th>
              <th className="th text-end">
                {t("inStock")}
                <SortBtn col="quantity" />
              </th>
              <th className="th text-end">{t("buyPrice")}</th>
              <th className="th text-end">{t("sellPrice")}</th>
              <th className="th text-center">{t("stockHistory")}</th>
              {isAdmin && <th className="th text-end">{t("by")}</th>}
          ...
          <tbody style={{ borderColor: "#F0EAE3" }}>
            {loading ? (
              ...
              filteredItems.map((item: IItem, idx: number) => {
                const uc = unitColor[item.unit || "pcs"] || {
                  bg: "#EBF5FB",
                  color: "#2980B9",
                  border: "#AED6F1",
                };
                      ...
                      <td className="td text-center">
                        <button
                          ...
                          style={{
                            ...
                            border: "1px solid #E5DDD5",
                            background:
                              expandedItemId === item._id ? "#FEF5E7" : "#fff",
                            color:
                        ...
                        >
                          ...
                          {item.batches?.length || 0} {t("entries")}
                        </button>
                      </td>
                      {isAdmin && (
                        <td className="td text-end">
