...
export default function EditSalePage() {
  ...
  useEffect(() => {
    if (status === "unauthenticated") {
      ...
      const canEdit =
        isAdmin || (session?.user?.permissions as any)?.sales?.edit;
      if (!canEdit) {
        router.push("/sales");
      }
    }
    ...
    const load = async () => {
      try {
        ...
        if (sr.success) {
          ...
          setSelCustomer({
            value: s.customerId,
            label: `${s.customerName} (${s.customerNumber})`,
            data: {
              _id: s.customerId,
              name: s.customerName,
          ...
          setDeliveryDate(
            s.deliveryDate ? formatDateInput(s.deliveryDate) : "",
          );
          setDeliveryAddress(s.deliveryAddress || "");

          // Fetch full customer info for mobile/address
          const custRes = await fetch(`/api/customers/${s.customerId}`).then(
            (r) => r.json(),
          );
          if (custRes.success) {
  ...
  const itemOptions: ISelectOption[] = items.map((i) => ({
    value: i._id,
    label: `${i.name} — ${formatCurrency(i.salesAmount)}`,
    data: i,
  }));
  ...
  return (
    <div className="page-container max-w-4xl">
      <TopBar title={t("editSale")} subtitle={`Updating record #${id}`} />
      ...
      <div className="card p-6 flex flex-col gap-6">
                  ...
                  <tr key={`${c.itemId}-${idx}`} className="align-top">
                    ...
                    <td className="td">
                      <input
                        ...
                        value={c.batch}
                        readOnly
                        className="w-full px-2 py-1 text-[10px] text-end border border-gray-100 bg-gray-50 rounded text-gray-400 font-mono"
                      />
                    </td>
                    <td className="td">
                      <input
                        ...
                        onChange={(e) =>
                          setCart((prev) =>
                            prev.map((it, i) =>
                              i === idx
                                ? { ...it, manufacturingDate: e.target.value }
                                : it,
                            ),
                          )
                        }
                        className="w-32 px-1 py-1 text-[10px] text-end border border-gray-200 rounded focus:ring-1 focus:ring-emerald-500"
                      />
                    </td>
                    <td className="td">
                      <input
                    ...
                    <td className="td text-end">
                      <input
                        ...
                        className="w-20 px-2 py-1 text-[10px] text-end border border-gray-200 rounded focus:ring-1 focus:ring-emerald-500"
                      />
                    </td>
                    ...
                    <td className="td text-end font-semibold text-gray-800">
                      {c.isFOC ? (
                        ...
                        <input
                          ...
                          className="w-20 px-2 py-1 text-[10px] text-end font-bold border border-gray-200 rounded focus:ring-1 focus:ring-emerald-500"
                        />
                      )}
                    </td>
                    <td className="td text-center">
        ...
        <div className="flex flex-col items-end gap-1 text-sm border-t border-gray-100 pt-4 font-medium">
          ...
          <div className="flex gap-10 text-amber-600">
          ...
          <div className="flex gap-10 text-lg font-bold text-gray-800 border-t border-gray-100 pt-2 mt-2">
        ...
        <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-6 border-t border-gray-100">
          ...
          <Button
            ...
            loading={saving}
            icon={<Save size={16} />}
            className="bg-emerald-600 hover:bg-emerald-700 text-white border-emerald-600 w-full sm:w-auto"
          >
            {t("updateSale")}
          </Button>
        </div>
      ...
      <ConfirmModal
        ...
        title={t("updateSale")}
        message={t("inventoryQuantitiesWillBeReconciled")}
