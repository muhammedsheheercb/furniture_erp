"use client";
import {
  Plus, Search, Printer, ClipboardList,
  Trash2, Wallet, Receipt,
} from "lucide-react";
    ...
    try {
      if (editSale?._id) {
        ...
        if (res.data.success) {
          ...
          setModalOpen(false); setEditSale(null);
          fetchSales();
        }
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-3xl font-extrabold text-[#1A1210]">Sales</h2>
      ...
      <ConfirmModal
        ...
        message="Are you sure you want to delete this order? This will revert stock quantities."
        loading={deleting}
      />

      <UpdateBalanceModal
        open={balanceModalOpen}
        onClose={() => { setBalanceModalOpen(false); setBalanceSale(null); }}
                        ...
                        {quotations.length === 0 ? (
                          <tr>
                            <td colSpan={5} className="py-10 text-center text-[#7A6055]">
                              No quotations ready for conversion
                            </td>
                          </tr>
                        ) : quotations.map(q => (
                          <tr key={q._id} className="hover:bg-[#FAF8F6] transition-colors">
                        ...
                        {(() => {
                          const activeOrders = sales
                            .filter(item => {
                              ...
                              const delivered = item.deliveryStatus === "delivered" || item.status === "invoiced";
                          ...
                          if (activeOrders.length === 0) {
                            return (
                              <tr>
                                <td colSpan={8} className="py-10 text-center text-[#7A6055]">
                          ...
                          return activeOrders.map(item => {
                            ...
                            return (
                              <tr key={item._id} className="hover:bg-[#FAF8F6] transition-colors">
                                {/* Order # */}
                                ...
                                <td className="py-4 px-6">
                                  ...
                                  <div className="text-[10px] text-[#A89080]">
                                    {item.customerNumber && <span className="mr-2 font-mono">{item.customerNumber}</span>}
                                ...
                                <td className="py-4 px-6 text-sm text-emerald-700 font-semibold">
                                  <CurrencySymbol /> {(item.advancePaid || 0).toLocaleString()}
                                </td>

                                {/* Balance */}
                                <td className="py-4 px-6 text-sm font-bold">
                                  <span className={bal > 0 ? "text-rose-600" : "text-emerald-600"}>
                                    <CurrencySymbol /> {Math.max(0, bal).toLocaleString()}
                                  </span>
                                </td>
                                  ...
                                  <div className="flex justify-end items-center gap-1">
                                    ...
                                    {canEdit && bal > 0 && (
                                      <Button
                                        ...
                                        onClick={() => { setBalanceSale(item); setBalanceModalOpen(true); }}
                                      >
                                        <Wallet size={15} />
                                      </Button>
                                    )}
                      ...
                      <tbody className="divide-y divide-[#F0EBE5]">
                        {(() => {
                          ...
                          if (invoices.length === 0) {
                            return (
                              <tr>
                                <td colSpan={6} className="py-10 text-center text-[#7A6055]">
                          ...
                          return invoices.map(item => (
                            <tr key={item._id} className="hover:bg-[#FAF8F6] transition-colors">
                              ...
                              <td className="py-4 px-6">
                                <div className="text-sm font-semibold text-[#1A1210]">{item.customerName}</div>
                                {item.customerNumber && (
                                  <div className="text-[10px] font-mono text-[#A89080]">{item.customerNumber}</div>
                                )}
                              </td>
                              <td className="py-4 px-6 text-sm font-bold text-[#1A1210]">
                                <CurrencySymbol /> {item.total.toLocaleString()}
