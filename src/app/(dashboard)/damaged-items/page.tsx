...
export default function DamagedItemsPage() {
  ...
  useEffect(() => { fetchDamagedItems(); fetchItems(); }, [page]);
  ...
  const handleItemSelect = (item: IItem) => {
    setFormData({ ...formData, itemId: item._id, itemNumber: item.itemNumber, itemName: item.name, batch: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedItem) {
      let maxQty = formData.batch
        ? (selectedItem.batches?.find(b => b.batchNumber === formData.batch)?.quantity || 0)
      ...
      if (Number(formData.quantity) > maxQty) {
        return toast.error(`Cannot exceed available quantity (${maxQty} ${selectedItem.unit || "pcs"})`);
      }
    }
  ...
  const openCreate = () => {
    ...
    setFormData({ itemId: "", itemNumber: "", itemName: "", quantity: 1, batch: "", reason: "", date: new Date().toISOString().split("T")[0] ?? "" });
  ...
  return (
    <div className="page-container">
      ...
      >
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 800, color: "#1A1210", margin: 0 }}>Damaged / Defective Items</h1>
          <p style={{ fontSize: 13, color: "#7A6055", margin: "4px 0 0" }}>
            Track furniture damaged in transit, production, or showroom
          </p>
        </div>
        {canCreate && (
          <motion.button
      ...
      <div className="table-wrapper">
        <table className="w-full">
          <thead>
            <tr>
              <th className="th">Furniture Item</th>
              <th className="th text-right">Qty Lost</th>
              <th className="th text-center">Reason</th>
              <th className="th text-center">Date</th>
              {isAdmin && <th className="th text-center">Recorded By</th>}
              <th className="th text-right">Actions</th>
            </tr>
          </thead>
          <tbody style={{ borderColor: "#F0EAE3" }}>
            {loading ? (
              <tr><td colSpan={isAdmin ? 6 : 5} style={{ textAlign: "center", padding: "48px 0" }}><Spinner /></td></tr>
            ) : damagedItems.length === 0 ? (
              <tr>
                <td colSpan={isAdmin ? 6 : 5} style={{ textAlign: "center", padding: "64px 0" }}>
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                    <Ban size={36} color="#E5DDD5" />
                    <p style={{ color: "#A89080", fontSize: 14, margin: 0 }}>No damage records found</p>
                  </div>
                </td>
              </tr>
              ...
              >
                <td className="td">
                  <div style={{ fontWeight: 700, color: "#1A1210", fontSize: 13 }}>{item.itemName}</div>
                  <div style={{ fontFamily: "monospace", fontSize: 10, color: "#A89080" }}>{item.itemNumber}</div>
                </td>
                <td className="td text-right">
                  <span style={{
                    fontWeight: 800, fontSize: 16, color: "#C0392B",
                ...
                <td className="td text-center">
                  <span style={{
                    display: "inline-block", padding: "3px 12px", borderRadius: 20, maxWidth: 220,
                    fontSize: 11, fontWeight: 600, background: "#FDEDEC",
                    color: "#C0392B", border: "1px solid #F5B7B1",
                    ...
                    {canEdit && (
                      <button
                        onClick={() => {
                          ...
                          setFormData({
                            itemId: item.itemId, itemNumber: item.itemNumber,
                            itemName: item.itemName, batch: item.batch || "",
      ...
      <Modal
        ...
        title={editingDamage ? "Edit Damage Record" : "Record Damaged Furniture"}
      >
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            ...
            <Select
              ...
              value={formData.itemId
                ? { value: formData.itemId, label: `${formData.itemNumber} — ${formData.itemName}`, data: items.find(i => i._id === formData.itemId) }
                : null}
              onChange={(opt: any) => opt && handleItemSelect(opt.data)}
              placeholder="Search furniture item…"
              styles={{
                control: (b: any) => ({ ...b, border: "1.5px solid #E5DDD5", borderRadius: 8, fontSize: 13, background: "#FAF8F6", minHeight: 40 }),
          ...
          {selectedItemBatches.length > 0 && (
            <div>
              <label style={{ fontSize: 12, fontWeight: 600, color: "#5A4035", display: "block", marginBottom: 6, letterSpacing: "0.04em" }}>
                PURCHASE BATCH (Optional)
              </label>
              <Select
                options={selectedItemBatches.filter(b => b.quantity > 0).map(b => ({
                  value: b.batchNumber,
                ...
                isClearable
                placeholder="Select specific batch…"
                styles={{ control: (b: any) => ({ ...b, border: "1.5px solid #E5DDD5", borderRadius: 8, fontSize: 13, background: "#FAF8F6", minHeight: 40 }) }}
              />
            </div>
          )}
          ...
          <div>
            ...
            {!DAMAGE_REASONS.includes(formData.reason) && formData.reason && (
              <p style={{ fontSize: 11, color: "#C9A84C", marginTop: 4 }}>Custom reason entered</p>
            )}
          </div>

          <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, borderTop: "1px solid #F0EAE3", paddingTop: 14 }}>
            <button
              type="button"
            ...
            >{editingDamage ? "Update Record" : "Confirm Damage"}</button>
          </div>
        </form>
      </Modal>
      ...
      <ConfirmModal
        ...
        message="Delete this record? The item quantity will be restored to inventory."
        confirmLabel="Delete"
        loading={deleting}
      />
    </div>
