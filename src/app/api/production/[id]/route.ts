...
export async function PUT(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    if (status === "processing" && production.status === "pending" && items) {
        for (const config of items) {
            ...
            for (const bomItem of config.bom) {
                ...
                if (!batch) throw new Error(`Batch ${bomItem.batchNumber} not found for material ${bomItem.materialName}`);
                
                const totalNeeded = bomItem.quantity;
                if (batch.quantity < totalNeeded) {
                    throw new Error(`Insufficient stock for material ${bomItem.materialName} in batch ${bomItem.batchNumber}`);
                }
    ...
    if (status === "processing") {
        await Sale.findByIdAndUpdate(production.saleId, { status: "processing" }).session(dbSession);
    } else if (status === "finished") {
        await Sale.findByIdAndUpdate(production.saleId, { status: "delivered" }).session(dbSession);
    }

    // If status is finished, create delivery entry
    if (status === "finished") {
        const existingDelivery = await Delivery.findOne({ saleId: production.saleId }).session(dbSession);
        if (!existingDelivery) {
