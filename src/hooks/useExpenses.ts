...
export function useExpenses() {
  ...
  const createExpense = useCallback(async (form: IExpenseForm): Promise<boolean> => {
    try {
  ...
  const updateExpense = useCallback(async (id: string, form: Partial<IExpenseForm>): Promise<boolean> => {
    try {
      const res  = await fetch(`/api/expenses/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
      const data = await res.json();
      if (!data.success) throw new Error(data.error);
  ...
  return { expenses, total, totalPages, totalAmount, loading, fetchExpenses, createExpense, deleteExpense, updateExpense };
}
