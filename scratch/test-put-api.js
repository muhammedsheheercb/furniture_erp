...
async function main() {
  try {
    ...
    const pendingProd = await db.collection("productions").findOne({ status: "pending" });
    ...
    const res = await axios.put(`http://localhost:3000/api/production/${pendingProd._id}`, payload, {
      headers: {
